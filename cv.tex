...
\section*{Technical skills}
 ...
 \textbf{programming language} C++, Python $\bullet$ \textbf{web} JavaScript, TypeScript, NodeJS, HTML5, CSS3 $\bullet$ \textbf{source control} Git $\bullet$ \textbf{computer graphics api} Vulkan, WebGL $\bullet$ \textbf{3d format} glTF, FBX, OBJ $\bullet$ \textbf{database} PostgreSQL, MongoDB $\bullet$ \textbf{operating system} Windows, macOS and Linux (Bash scripting) $\bullet$ \textbf{tools} CMake, Vim, FFmpeg, ImageMagick $\bullet$ \textbf{software} Unity, Blender, Affinity Designer
