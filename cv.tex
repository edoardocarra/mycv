...
\section*{Technical skills}
 ...
 \textbf{programming language} C++, Python $\bullet$ \textbf{web} JavaScript, TypeScript, NodeJS $\bullet$ \textbf{source control} Git $\bullet$ \textbf{computer graphics api} Vulkan, OpenGL, GLFW $\bullet$ \textbf{3d format} glTF, FBX, OBJ $\bullet$ \textbf{database} PostgreSQL, MongoDB $\bullet$ \textbf{operating system} Windows (cmd/Batch scripting), macOS and Linux (Bash scripting) $\bullet$ \textbf{tools} CMake, Vim, FFmpeg, ImageMagick $\bullet$ \textbf{software} Unity, Blender, Affinity Designer
...
\years{2019} Edoardo Carra, Fabio Pellacini, \emph{SceneGit: A Practical System for Diffing and Merging 3D Environments}, ACM Transaction on Graphics (SIGGRAPH ASIA 2019), Conditionally Accepted\\
